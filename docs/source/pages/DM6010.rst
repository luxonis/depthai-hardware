...
The Y-adapter is used to **supply power to OAK cameras** through a USB-C connector. Note that in most cases, the host computer is
able to provide enough current (>1A) and you don't need a Y-adapter. The **Y-adapter is only needed** in cases where the host is not able
...
- In case you are using a small SBC like :ref:`Raspberry Pi <OAK & Raspberry Pi>`, and the OAK camera isn't the only USB device that's drawing power.

Older (pre-2022) versions of OAK cameras have a power barrel jack for supplying power (instead of the Y-adapter), while
newer OAK cameras only have a USB-C connector, which is why we engineered the Y-adapter.
