...
This calibration consists of three separate calibration parameters.  
...
view, change in the perspective of the captured images, rectification, and calculation of depth in stereo setup. Detailed description of the Camera 
...
Rectification parameters contain two rotation matrices containing roll, pitch, and yaw to be applied on the left and right camera of the stereo setup. 
This converts the frame into virtual frames where rows of the two images are aligned for feature matching.
...
* Intrinsics of a camera almost always need factory calibration. These absolutely must not change in the field. While there are field-calibration 
...
* Extrinsics are often field-calibratable. But this must not be relied on carte-blanche, as the user experience of such a field-calibration may not 
  be good for the product being built. And whether it is possible depends on what the camera is being pointed at in the field, since the system needs 
...
layouts in 2D and 3D, and/or screenshots of mechanical assemblies).
...
review. Many issues are difficult to notice in CAD, but easy/fast to notice when they are in an engineer's hand (and they can see that the mounting 
...
- Mount the cameras to rigid structures. 
    ...
    - If mounting to a PCB, ideally reinforce the PCB with mounting to a metal support near to where the cameras mount to the PCB, and/or have a not-thin PCB so it is less likely to bend.
