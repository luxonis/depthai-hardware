...
Compared to OAK-D, the OAK-D-Lite is smaller, lighter, and uses less power. Following are the main differences:

- Mono cameras have lower resolution (640x480 instead of 1280x800).
- There is no IMU (Inertial Measurement Unit) on the OAK-D-Lite, as most users don't need it. IMUs are currently hard to come by, meaning that single component increases price significantly, and we wanted to lower the price point as much as possible.
- There is no power jack, as most users just use the USB-C for power delivery, which provides 900mA at 5V and is enough for most use-cases. However, some functions (e.g. inference, video encoding) can lead to large current spikes, so there is a chance hosts like RPi won't be able to provide enough power. In that case, you should use :ref:`Y-adapter`.
...
color camera**. You should select FF if you are mounting the camera to something that vibrates. You should select AF when
you need things closer than ~50 cm to be in focus. More information can be found at :ref:`Auto-Focus vs Fixed-Focus`.
