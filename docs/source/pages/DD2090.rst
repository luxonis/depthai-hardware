...
If you have an `OAK-FFC-IR <https://shop.luxonis.com/collections/early-access/products/oak-ffc-ir>`__ compatible board (rev. R5M1E5 and up) and wish to use the IR module, you **must** first configure from which camera you want to have STROBE signal sent to the connector on the left (to which you connect the IR module). 
